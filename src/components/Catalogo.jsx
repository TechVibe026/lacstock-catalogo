...
function Catalogo() {
  ...
  const filtros = {
    ...
    tamanhos: searchParams.getAll('tamanho'),
    cores: searchParams.getAll('cor'),
    precoMaximo: Number(
      searchParams.get('preco') || precoPadrao,
    ),
  }
  ...
  const atualizarParametros = (alteracoes) => {
    ...
    Object.entries(alteracoes).forEach(([chave, valor]) => {
      ...
      if (Array.isArray(valor)) {
        valor.forEach((item) => {
          novosParametros.append(chave, item)
        })
  ...
  const setFiltros = (novoEstado) => {
    ...
    atualizarParametros({
      ...
      cor: resultado.cores,
      preco:
        resultado.precoMaximo < precoPadrao
  ...
  const quantidadeFiltros =
    ...
    (filtros.precoMaximo < precoPadrao ? 1 : 0)
  ...
  const compartilharSelecao = async () => {
        ...
        <div className="catalog__header">
          <div>
            <span className="catalog__eyebrow">
              <TextReveal delay={100} speed={80}>
  CATÁLOGO
</TextReveal>
            </span>

            <h2>
  <TextReveal delay={650} speed={55}>
    Explore a LacStock
  </TextReveal>
</h2>
          </div>
...
<div className="catalog__grid">
  {produtosFiltrados.map((produto, index) => {
    const posicaoNaLinha = index % 3

    return (
      <Reveal
        key={produto.id}
        direction="left"
        delay={posicaoNaLinha * 220}
      >
        <ProductCard produto={produto} />
          ...
          <div className="catalog__empty">
            ...
            <p>
              Tente remover algum filtro ou explorar
              outras opções.
            </p>
